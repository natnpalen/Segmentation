...
| `ShavedBoneMode` | `false` | For scans of machined specimens — e.g. metacarpals with parts of the cortical bone shaved flat for 3-point bending. Lowers the minimum bone size to 150 mm^3 (unless `MinBoneVolMM3` is set explicitly) and uses gentler surface cleanup so thin cortical plates aren't eroded away. Surface-cleanup HU thresholds are held at fixed low floors (~70-100 HU) instead of scaling with bone density, so cancellous bone exposed on machined cut faces is kept rather than scrubbed off as "tissue". Each bone's console line reports the unclaimed bone-like material within 1mm of the final mask, so dropped surface bone is visible in the log. |
