...
for bi = 1:n_bones
    ...
    try
        ...
        % Label above the bone (not inside it), black on a white box with
        % the bone's color as the border. Alternate the height so labels
        % of neighboring bones don't overlap each other.
        z_top = bones{bi}.bbox(6) * spacing(3);
        z_lbl = z_top + 4 + 8 * mod(bi, 2);
        text(cm(2), cm(1), z_lbl, lbl, 'FontSize', 9, 'FontWeight', 'bold', ...
            'HorizontalAlignment', 'center', 'Color', 'k', ...
            'BackgroundColor', 'w', 'EdgeColor', colors(bi,:), ...
